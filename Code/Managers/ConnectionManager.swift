...
public class ConnectionManager: NSObject {
  ...
  */
  ...
  - Parameter newLocation: The position to move to.
  - Parameter offset: An additional offset, usually the position of the parent view in the workspace
  view.
  */
  public func moveConnection(
    connection: Connection, toNewLocation newLocation: WorkspacePoint, offset: WorkspacePoint) {
      moveConnection(connection,
        toNewX: newLocation.x + offset.x,
        newY: newLocation.y + offset.y)
  }
  ...
  */
  internal static func couldConnect(
  ...
  */
  private func moveConnection(connection: Connection, toNewX newX: CGFloat, newY: CGFloat) {
    ...
    internal func searchForClosestConnectionTo(connection: Connection, maxRadius: CGFloat)
      -> Connection? {
        ...
        while (pointerMin >= 0 && isInYRangeForIndex(pointerMin, baseY, maxRadius)) {
          ...
          if ConnectionManager.couldConnect(connection, toConnection: temp, maxRadius: bestRadius) {
            bestConnection = temp
      ...
      -> [Connection] {
        ...
        while (pointerMin >= 0 && isInYRangeForIndex(pointerMin, baseY, maxRadius)) {
          ...
          if ((!connection.connected || !temp.connected) &&
            ConnectionManager.couldConnect(connection, toConnection: temp, maxRadius: maxRadius)) {
