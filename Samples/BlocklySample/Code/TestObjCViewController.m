...
- (void)viewDidLoad {
  ...
  [blockBuilder setPreviousConnectionWithEnabled:true typeChecks:nil error:nil];
  ...
  // Create workspace
  BKYBlock *defaultBlock = [blockBuilder buildWithShadow:NO error:nil];
  BKYWorkspace *workspace = [[BKYWorkspace alloc] init];
  [workspace addBlockTree:defaultBlock error:nil];

  // Create toolbox
  BKYBlock *toolboxBlock = [blockBuilder buildWithShadow:NO error:nil];
