...
public final class DefaultBlockLayout: BlockLayout {
  ...
  public override func performLayout(includeChildren: Bool) {
    ...
    for layout in layouts {
      ...
      if backgroundRow != nil && previousInputLayout == nil && inputLayout?.input.type != .statement
      {
        // Row was already created for mutators, don't make a new one. Append to it.
      } else if backgroundRow == nil || // First row
      ...
      let xOffsetRelativeToLeftEdge = layout.relativePosition.x - outputPuzzleTabXOffset
        ...
        if defaultInputLayout.input.type == .statement {
          minimalStatementWidthRequired =
            max(minimalStatementWidthRequired,
                xOffsetRelativeToLeftEdge + defaultInputLayout.minimalStatementWidthRequired)
        } else if !block.inputsInline {
          minimalFieldWidthRequired =
            max(minimalFieldWidthRequired,
                xOffsetRelativeToLeftEdge + defaultInputLayout.minimalFieldWidthRequired)
        }
      } else if let mutatorLayout = layout as? MutatorLayout,
        !block.inputsInline
      {
        minimalFieldWidthRequired =
          max(minimalFieldWidthRequired, xOffsetRelativeToLeftEdge + mutatorLayout.totalSize.width)
      }
    ...
    for backgroundRow in background.rows {
      ...
      backgroundRow.updateRenderProperties(minimalRowWidth: minimalWidthRequired,
                                           leftEdgeOffset: outputPuzzleTabXOffset)
    }
