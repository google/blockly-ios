...
extension DefaultBlockLayout {
  ...
  */
  ...
  public final class BackgroundRow: NSObject {
    ...
    - parameter leftEdgeOffset: The left edge offset relative to the block background. NOTE: This
     value is only used for rows containing statement/non-inline value inputs.
    */
    internal func updateRenderProperties(minimalRowWidth: CGFloat, leftEdgeOffset: CGFloat) {
      if layouts.isEmpty {
        return
      }
      ...
      if let lastInputLayout = layouts.last as? DefaultInputLayout {
        if lastInputLayout.input.type == .statement {
          ...
          self.rightEdge =
            lastInputLayout.relativePosition.x - leftEdgeOffset + lastInputLayout.rightEdge
