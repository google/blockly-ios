...
class WorkspaceTest: XCTestCase {
  ...
  func testRemovingInputFromBlock() {
    // Create block builder
    let blockBuilder = BlockBuilder(name: "block")
    blockBuilder.inputBuilders.append(InputBuilder(type: .value, name: "input"))
    BKYAssertDoesNotThrow { try blockBuilder.setOutputConnection(enabled: true) }

    guard
      let parent = BKYAssertDoesNotThrow({
        try blockBuilder.makeBlock(shadow: false, uuid: "parent")
      }),
      let child = BKYAssertDoesNotThrow({
        try blockBuilder.makeBlock(shadow: false, uuid: "child")
      }),
      let shadowChild = BKYAssertDoesNotThrow({
        try blockBuilder.makeBlock(shadow: true, uuid: "shadowChild")
      }),
      let parentInput = parent.firstInput(withName: "input")?.connection else
    {
      XCTFail("Could not build blocks")
      return
    }

    // Connect child blocks to parent
    BKYAssertDoesNotThrow {
      try parentInput.connectTo(child.outputConnection)
      try parentInput.connectShadowTo(shadowChild.outputConnection)
    }

    XCTAssertTrue(parentInput.connected)
    XCTAssertEqual(child.outputConnection, parentInput.targetConnection)
    XCTAssertTrue(parentInput.shadowConnected)
    XCTAssertEqual(shadowChild.outputConnection, parentInput.shadowConnection)

    // Add block tree to workspace
    BKYAssertDoesNotThrow { try _workspace.addBlockTree(parent) }
    XCTAssertEqual(3, _workspace.allBlocks.count)

    // Remove input from parent
    parent.removeInput(parent.inputs[0])

    // Both child blocks should have been disconnected from the parent, but only the shadow child
    // block should have been automatically deleted from the workspace
    XCTAssertEqual(2, _workspace.allBlocks.count)
    XCTAssertNotNil(_workspace.allBlocks["parent"])
    XCTAssertNotNil(_workspace.allBlocks["child"])
    XCTAssertNil(_workspace.allBlocks["shadowChild"])
    XCTAssertFalse(parentInput.connected)
    XCTAssertFalse(parentInput.shadowConnected)
  }
}
