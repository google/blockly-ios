...
class ConnectionManagerTest: XCTestCase {
  ...
  func testConnectionManagerMoveTo() {
    ...
    manager.moveConnection(conn, toNewLocation: WorkspacePointMake(moveX, moveY), offset: offset)
  ...
  func testConnectionManagerIsConnectionAllowed() {
    ...
    XCTAssertTrue(ConnectionManager.couldConnect(one, toConnection: two, maxRadius: 20.0))
    // Move connections farther apart
    two.position.x = 100
    two.position.y = 100
    XCTAssertFalse(ConnectionManager.couldConnect(one, toConnection: two, maxRadius: 20.0))

    // Don't offer to connect an already connected left (male) value plug to
    // an available right (female) value plug.
    let three = createConnection(0, 0, .OutputValue)
    XCTAssertTrue(ConnectionManager.couldConnect(one, toConnection: three, maxRadius: 20.0))
    let four = createConnection(0, 0, .InputValue)
    if (try? three.connectTo(four)) == nil {
      XCTFail("Could not connect connections 3 and 4")
    }
    XCTAssertFalse(ConnectionManager.couldConnect(one, toConnection: three, maxRadius: 20.0))

    // Don't connect two connections on the same block
    two.sourceBlock = one.sourceBlock
    XCTAssertFalse(ConnectionManager.couldConnect(one, toConnection: two, maxRadius: 1000.0))
  }
  ...
  func testYSortedListOrdered() {
    ...
    // quasi-random
    let xCoords: [CGFloat] = [-29, -47, -77, 2, 43, 34, -59, -52, -90, -36, -91, 38, 87, -20, 60, 4, -57,
    ...
    let yCoords: [CGFloat] = [-81, 82, 5, 47, 30, 57, -12, 28, 38, 92, -25, -20, 23, -51, 73, -90, 8, 28,
