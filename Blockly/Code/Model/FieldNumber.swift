...
public final class FieldNumber: Field {
  ...
  public static let DEFAULT_MINIMUM_VALUE = -DBL_MAX
  public static let DEFAULT_MAXIMUM_VALUE = DBL_MAX
  public static let DEFAULT_PRECISION = DBL_TRUE_MIN // DBL_MIN
  public static let MAXIMUM_FRACTION_DIGITS = 100
  ...
  */
  ...
  public var textValue: String {
    return _textNumberFormatter.stringFromNumber(NSNumber(double: value)) ?? String(value)
  }
  ...
  private let _textNumberFormatter = NSNumberFormatter()
  ...
  public override func setValueFromSerializedText(text: String) throws {
    ...
    guard let value = _serializedNumberFormatter.numberFromString(text)?.doubleValue else {
      throw BlocklyError(.XMLParsing,
        "Could not parse value [`\(text)`] into a number. The value must be a valid number.")
    }

    if !value.isFinite {
      throw BlocklyError(.XMLParsing, "Value [`\(text)`] cannot be NaN or infinite.")
    }
  ...
  private func updateNumberFormatter() {
    ...
    _textNumberFormatter.minimumFractionDigits = minimumFractionDigits
    _textNumberFormatter.maximumFractionDigits = maximumFractionDigits
