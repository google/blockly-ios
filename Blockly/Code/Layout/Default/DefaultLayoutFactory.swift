...
extension DefaultLayoutFactory: LayoutFactory {
  ...
  public func layoutForBlock(block: Block, engine: LayoutEngine) throws -> BlockLayout {
    guard let defaultEngine = engine as? DefaultLayoutEngine else {
      throw errorForWrongLayoutEngine(engine)
    }
    return DefaultBlockLayout(block: block, defaultEngine: defaultEngine)
  }
  ...
  public func layoutForBlockGroupLayout(engine engine: LayoutEngine) throws -> BlockGroupLayout {
    ...
    return DefaultBlockGroupLayout(defaultEngine: defaultEngine)
  }
  ...
  public func layoutForInput(input: Input, engine: LayoutEngine) throws -> InputLayout {
    ...
    return try DefaultInputLayout(input: input, defaultEngine: defaultEngine, defaultFactory: self)
  }
  ...
  public func layoutForField(field: Field, engine: LayoutEngine) throws -> FieldLayout {
    ...
    if let closure = _fieldLayoutCreators[fieldTypeHash] {
      return try closure(field: field, engine: defaultEngine)
    }

    throw BlocklyError(.LayoutNotFound, "Could not find layout for \(field.dynamicType)")
  }

  private func errorForWrongLayoutEngine(engine: LayoutEngine) -> BlocklyError {
    return BlocklyError(.IllegalState,
      "Expected `engine` of type `DefaultLayoutEngine`, but was of type `\(engine.dynamicType)`")
  }
}
