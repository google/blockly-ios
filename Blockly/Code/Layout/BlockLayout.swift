...
open class BlockLayout: Layout {
  ...
  */
  open func append(inputLayout: InputLayout) {
  ...
  */
  @discardableResult
  open func removeInputLayoutAt(index: Int) -> InputLayout {
  ...
  */
  internal func inputLayoutBefore(layout: InputLayout) -> InputLayout? {
  ...
  */
  internal func inputLayoutAfter(layout: InputLayout) -> InputLayout? {
    for i in 0 ..< inputLayouts.count {
      if inputLayouts[i] == layout {
...
extension BlockLayout: ConnectionHighlightDelegate {
  public func didChangeHighlightFor(connection: Connection) {
