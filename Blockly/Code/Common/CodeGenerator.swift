...
public final class CodeGenerator: NSObject {
  ...
  */
   ...
   */
  fileprivate func contentsOf(bundledFile: BundledFile) throws -> String {
...
extension CodeGenerator: WKNavigationDelegate {
  public func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
    ...
    do {
      ...
      jsScripts.append(try contentsOf(bundledFile: (file: CodeGenerator.CODE_GENERATOR_BRIDGE_JS,
        bundle: Bundle(for: CodeGenerator.self))))

      // Load JS dependencies
      for bundledFile in jsCoreDependencies {
        jsScripts.append(try contentsOf(bundledFile: bundledFile))
      }
      ...
      for bundledFile in jsonBlockDefinitions {
        let fileContents = try contentsOf(bundledFile: bundledFile)
