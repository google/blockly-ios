...
public class BlockBumper: NSObject {
  ...
  /// system unit. This value is read from `self.workspaceLayout?.config.floatFor()` and defaults to `0`
  /// if no value is specified.
  private var bumpDistance: CGFloat {
    return workspaceLayout?.config.floatFor(LayoutConfig.BumpDistance) ?? 0
  }
