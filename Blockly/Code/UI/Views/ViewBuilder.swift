...
extension ViewBuilder: LayoutHierarchyListener {
  public func layout(layout: Layout, didAddChildLayout childLayout: Layout) {
  ...
  public func layout(
    layout: Layout, didTransferChildLayout childLayout: Layout,
    toNewParentLayout newParentLayout: Layout)
  {
    guard
      let parentView = ViewManager.sharedInstance.findViewForLayout(newParentLayout),
      let childView = ViewManager.sharedInstance.findViewForLayout(childLayout) else
    {
      return
    }

    // This transfers the childView over to parentView
    parentView.addSubview(childView)
  }
}