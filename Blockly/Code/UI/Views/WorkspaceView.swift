  ...
  */
  ...
  private func updateCanvasSizeFromLayout() {
    ...
    if allowCanvasPadding {
      ...
      contentPadding.top = scrollView.bounds.size.height + canvasPadding.top
      contentPadding.leading = scrollView.bounds.size.width + canvasPadding.leading
      contentPadding.bottom = scrollView.bounds.size.height + canvasPadding.bottom
      contentPadding.trailing = scrollView.bounds.size.width + canvasPadding.trailing
    }
    ...
    if layout.engine.rtl {
      ...
      if oldBlockGroupFrame.size == CGSizeZero {
        // If the previous block group size was CGSizeZero, it was never actually positioned.
        ...
        oldBlockGroupFrame.origin.x =
          scrollView.bounds.size.width - (allowCanvasPadding ? canvasPadding.leading : 0)
      }
      ...
      scrollView.contentOffset.x +=
        scrollView.blockGroupView.frame.origin.x + scrollView.blockGroupView.frame.width -
        (oldBlockGroupFrame.origin.x + oldBlockGroupFrame.width)
      scrollView.contentOffset.y +=
        scrollView.blockGroupView.frame.origin.y - oldBlockGroupFrame.origin.y
    } else {
      let blockGroupOrigin = CGPointMake(contentPadding.leading, contentPadding.top)
      let oldBlockGroupOrigin = scrollView.blockGroupView.frame.origin
      scrollView.blockGroupView.frame = CGRectMake(
        blockGroupOrigin.x, blockGroupOrigin.y, blockGroupSize.width, blockGroupSize.height)
      ...
      // The block group origin may have changed since the last call to
      // `updateCanvasSizeFromLayout()`, so we need to adjust the content offset for this too.
      scrollView.contentOffset.x += scrollView.blockGroupView.frame.origin.x - oldBlockGroupOrigin.x
      scrollView.contentOffset.y += scrollView.blockGroupView.frame.origin.y - oldBlockGroupOrigin.y
    }
  ...
  private func removeExcessScrollSpace() {
    ...
    if layout.engine.rtl {
      ...
      let originX = max(canvasPadding.trailing,
        scrollView.bounds.size.width - scrollView.blockGroupView.frame.size.width -
          canvasPadding.leading)
      idealBlockGroupFrame = CGRectMake(
        originX,
        canvasPadding.top,
        scrollView.blockGroupView.frame.size.width + canvasPadding.leading,
        scrollView.blockGroupView.frame.size.height + canvasPadding.bottom)
    } else {
      idealBlockGroupFrame = CGRectMake(canvasPadding.leading, canvasPadding.top,
        scrollView.blockGroupView.frame.size.width + canvasPadding.trailing,
        scrollView.blockGroupView.frame.size.height + canvasPadding.bottom)
    }

    // Remove excess left space
    let leftExcessSpace = scrollView.blockGroupView.frame.origin.x - idealBlockGroupFrame.origin.x
    ...
    let rightExcessSpace = scrollView.contentSize.width -
      (scrollView.blockGroupView.frame.origin.x + idealBlockGroupFrame.size.width)

    if rightExcessSpace > 0 &&
      (scrollView.contentOffset.x + scrollView.bounds.size.width) <= scrollView.contentSize.width
    {
      let adjustment = min(rightExcessSpace,
        scrollView.contentSize.width - (scrollView.contentOffset.x + scrollView.bounds.size.width))
      scrollView.contentSize.width -= adjustment
    }

    // Remove excess top space
    let topExcessSpace = scrollView.blockGroupView.frame.origin.y - idealBlockGroupFrame.origin.y
    ...
    let bottomExcessSpace = scrollView.contentSize.height -
      (scrollView.blockGroupView.frame.origin.y + idealBlockGroupFrame.size.height)
