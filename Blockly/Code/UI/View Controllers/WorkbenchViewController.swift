...
public class WorkbenchViewController: UIViewController {
  ...
  */
  ...
  public override func loadView() {
    ...
    let panGesture = BlocklyPanGestureRecognizer(targetDelegate: self, action: nil,
      destinationView: workspaceView.scrollView.containerView)
    panGesture.delegate = self
    workspaceViewController.view.addGestureRecognizer(panGesture)

    let toolboxGesture = BlocklyPanGestureRecognizer(targetDelegate: self, action: nil,
      destinationView: workspaceView.scrollView.containerView)
    toolboxGesture.delegate = self
    toolboxCategoryViewController.view.addGestureRecognizer(toolboxGesture)

    let trashGesture = BlocklyPanGestureRecognizer(targetDelegate: self, action: nil,
      destinationView: workspaceView.scrollView.containerView)
                                                  ...
                                                  touchPosition: CGPoint, touchIndex: Int, block: BlockView, state: BlocklyPanGestureState)
  {
    ...
    if state == .Began {
      ...
      if inToolbox {
        ...
        gesture.updateBlock(newBlock, forTouchIndex: touchIndex)
        blockView = newBlock
      } else if inTrash {
  ...
  public func gestureRecognizerShouldBegin(gestureRecognizer: UIGestureRecognizer) -> Bool {
    if let panGestureRecognizer = gestureRecognizer as? BlocklyPanGestureRecognizer
    {
      if gestureRecognizer.view == toolboxCategoryViewController.view {
        ...
        let velocity = panGestureRecognizer.firstTouchDeltaInView(panGestureRecognizer.view)
