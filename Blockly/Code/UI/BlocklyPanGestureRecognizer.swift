...
public class BlocklyPanGestureRecognizer: UIGestureRecognizer {
  ...
  /**
   ...
   - Parameter destinationView: The destination `UIView` where the dragged blocks will end up. Note:
     This is the workspace they started on, by default.
   */
  public init(targetDelegate: BlocklyPanGestureDelegate, destinationView: UIView)
  {
   ...
   */
  public func isTouchingView(view: UIView) -> Bool {
    for touch in _touches {
      let touchPosition = touch.locationInView(view)
      if CGRectContainsPoint(view.bounds, touchPosition) {
        return true
      }
    }
